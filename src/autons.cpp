...
void RedLeftFourRushWing() {
  ...
  chassis.pid_odom_set({{{15_in, 31_in}, fwd, DRIVE_SPEED}},
                       true);
  chassis.pid_wait(); 
  chassis.pid_turn_set(90_deg, TURN_SPEED);
  chassis.pid_wait();          
  chassis.pid_drive_set(7_in, 127, false); // Alligner Lock
  chassis.pid_wait();  // Long goal 

    //Scoring
  lever.tare_position(); 
    gate.set(true);
    lever.move_velocity(127); 

    
    while (lever.get_position() < 95.0) {
        pros::delay(10); // Check every 10ms for high precision
    }
    pros::c::delay(1000);
    ...
    while (lever.get_position() > 2.0) {
        pros::delay(10);
    }
    
  gate.set(false);
  lever.move_voltage(0);  // Turn the intake off
  intake.move(270);
  matchLoad.set(true);
  ...
  chassis.pid_wait_until_index(1);  
  pros::delay(2000);
  chassis.pid_odom_set({{{30_in, -5_in}, fwd, DRIVE_SPEED}},
                       true);
  chassis.pid_wait();
  chassis.pid_turn_set(120_deg, TURN_SPEED);
  chassis.pid_wait();          
...
//   setDescoreAngle(true);
  ...
  chassis.pid_odom_set({{{26_in, 19_in}, rev, DRIVE_SPEED}},
                       true);
  chassis.pid_wait();
